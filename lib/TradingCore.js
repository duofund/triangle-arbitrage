...
TradingCore.prototype.trading = async function (candidate) {
    ...
    if (this._paperOnly) {
        // utils.computeReturn(candidate.id, tradeObj, {})
...
TradingCore.prototype.posCycleArbitrage = function (candidate) {
    ...
    trade1.quantity = this.computeMiniQuantity(trade1.symbol, this.computeQuantity(trade1, baseQuote.A));
...
TradingCore.prototype.negCycleArbitrage = function (candidate) {
    ...
    trade1.quantity = this.computeMiniQuantity(trade1.symbol, this.computeQuantity(trade1, quoteMid.B));
...
TradingCore.prototype.computeQuantity = function (trade, vol) {
    if (trade.symbol.indexOf(process.env.binanceStartingPoint) === 0) {
        return this._mainCoinQuantityLimit
    }

    let stands = this._mainCoinQuantityLimit.div(trade.price)
    const baseQty = this._percentageOfBestQuantity * vol

    // if stands bigger then baseQty, then use baseQty (80% of base quantity)
    if (stands < baseQty || baseQty < 0.2 * stands) {
        return stands
    }
...
TradingCore.prototype.asyncQueryOrder = async function (trade, orderId, cid) {
    ...
    try {
        ...
        console.log('Get error:', e, 'with:', trade, 'orderId:', orderId)
    }

...
TradingCore.prototype.trading = async function (candidate) {
    ...
    try {
        ...
        const trade1Promise = new Promise(resolve => {
            self.asyncNewOrder({ ...trade1 }, candidate.id + ' trade1 ' + candidate.ts, true).then(res => {
                resolve(res)
            })
        })
        ...
        console.log(`%c <${candidate.id}> Arbitrage Success:`, 'color: blue', { tradeRes1, tradeRes2, tradeRes3 });
...
TradingCore.prototype.convertOrderToMarketPrice = async function (trade, cid) {
    ...
    // query price now
    const currentTicker = await this._exchange.tickerPrice(trade.symbol)

    console.log('currentTicker:', currentTicker)

    const tmpPrice = trade.price
    trade.price = trade.side === 'BUY' ? currentTicker.a : currentTicker.b

    console.log(`<${cid}> Change to current ticker price from: ${tmpPrice} to: ${trade.price}`, )
    // delete newTrade.timeInForce
    // delete newTrade.price

    return await this.asyncNewOrder(trade, cid, false)
}

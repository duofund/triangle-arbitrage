...
TradingCore.prototype.trading = async function (candidate) {
    const self = this
    const { trade1, trade2, trade3 } = self.posCycleArbitrage(candidate)
    // const { trade1, trade2, trade3 } = self.negCycleArbitrage(candidate)

    console.log(`<${candidate.id}> ==========  ${trade1.s} | ${trade2.s} | ${trade3.s}  ===========`)
    // console.log(`<${candidate.id}> trade:`, trade1.tradeInfo, trade2.tradeInfo, trade3.tradeInfo)

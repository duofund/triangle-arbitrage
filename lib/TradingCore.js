...
TradingCore.prototype.processQueue = async function(queue, stream){
  ...
  for (let i=0;i<keys.length;i++){
    ...
    console.log(`==========  ${cand.a.s} | ${cand.b.s} | ${cand.c.s}  ===========`)

    const posOrNeg = this.checkPosNeg(cand)
    ...
    if (cand.hits >= this._minHitsThreshold){
      ...
      // let posOrNeg = this.checkPosNeg(cand)
      //
      // if (!posOrNeg) {
      //   return
      // }

      cand.posOrNeg = posOrNeg
      cand.isTrade = true
      cand.id = this.cid
      this.cid = this.cid + 1;
...
TradingCore.prototype.checkPosNeg = function (candidate) {
    ...
    let quote = this._arbitrage.start
    let mid = a.s.substring(quote.length, a.s.length)// this._arbitrage.paths.filter(path => symbols.indexOf(quote + path) >= 0)[0]
    let base = markets.filter(market => market.stepFrom !== quote && market.stepFrom !== mid)[0].stepFrom
    ...
    const baseMidBuy = marketsMap[base + mid].b
    const baseMidSell = marketsMap[base + mid].a
    const quoteMidBuy = marketsMap[quote + mid].b
    const quoteMidSell = marketsMap[quote + mid].a
    const baseQuoteBuy = marketsMap[base + quote].b
    const baseQuoteSell = marketsMap[base + quote].a
    ...
    if (color === 'red') {
        console.log(`%c <${candidate.id}> posIndex: ${posIndex} | negIndex: ${negIndex} | allFee: ${allFee}`, `background-color: ${color}; color: white;`)
    }

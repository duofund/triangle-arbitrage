...
function TradingCore(ctrl) {
  ...
  this._tradeTime = tradeOpts.tradeTime;
  this.cid = 1001;
...
TradingCore.prototype.trading = async function (candidate) {
    ...
    try {
        ...
        console.error('Neg or Pos compute error:', e)
    }
    ...
    if (trade1Qty < trade1.quantity * 0.5 || trade2Qty < trade2.quantity * 0.5 || trade3Qty < trade3.quantity * 0.5) return console.log(`<${candidate.id}> Not enough quantity!`)

    if (this._tradeTime <= 0) return console.log(`<${candidate.id}> Out of Trading Time`)

    this._tradeTime = this._tradeTime - 1

    if (this._paperOnly) return this.paperTradeCore.arbitrage(trade1, trade2, trade3, candidate);
...
TradingCore.prototype.executeAsyncTrading = async function (candidate, trade1, trade2, trade3, tradeObj) {
    ...
    const tradeRes1 = await this.asyncNewOrder({ ...trade1 }, candidate.id + ' trade1 ' + candidate.ts, false);
...
TradingCore.prototype.computeQuantity = function (trade1, trade2, trade3, { trade1Vol }, candidate) {
    trade1.quantity = this.computeMiniQuantity(trade1.symbol, QuantityTools.withFee.trade1(trade1, trade1Vol));
    trade2.quantity = this.computeMiniQuantity(trade2.symbol, QuantityTools.withFee.trade2(trade1, trade2))
    trade3.quantity = this.computeMiniQuantity(trade3.symbol, QuantityTools.withFee.trade3(trade2, trade3))
...
TradingCore.prototype.asyncQueryOrder = async function (trade, orderId, cid, isOnly=false) {
    ...
    try {
        orderStatus = await this._exchange.apis.fetchOrder(orderId, trade.symbol);

        // Check 3 time if order filled
        if (orderStatus.status !== 'closed' && trade.timeInForce !== 'IOC') {
            while (i > 0) {
                i--;
...
TradingCore.prototype.asyncNewOrder = async function (trade, cid, mustBeFilled = false, notDecrease = false) {
    ...
    try {
        ...
        console.log(`<${cid}> symbol: ${trade.symbol}, type: ${trade.type}, side: ${trade.side}, quantity: ${trade.quantity}, price: ${trade.price}`)
        const payload = await self._exchange.apis.createOrder(trade.symbol, trade.type, trade.side, trade.quantity, trade.price, {timeInForce: trade.timeInForce})
        console.log(`<${cid}> ${new Date().getTime()} Trade Placed:`, payload)

        if (payload.status !== 'closed') {
            orderStatus = await self.asyncQueryOrder(trade, payload.id, cid)
        } else {
            orderStatus = payload
        }
        ...
        if (orderStatus.status !== 'closed' && trade.timeInForce !== 'IOC') {
            ...
            try {
                await this.cancelOrder(cid, trade, payload.id)
            } catch (e) {
            ...
            if (mustBeFilled && orderStatus.status !== 'closed') {
...
TradingCore.prototype.convertOrderToMarketPrice = async function (trade, cid) {
    ...
    // query price now
    const currentTicker = await this._exchange.apis.fetchTicker(trade.symbol)
...
TradingCore.prototype.cancelOrder = async function (cid, trade, orderId) {
    await this._exchange.apis.cancelOrder(orderId, trade.symbol)

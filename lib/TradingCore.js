...
TradingCore.prototype.trading = async function (candidate) {
    ...
    let { trade1, trade2, trade3, trade1Qty } = tradeObj

    // if best quantity 1 less then we 80% of quantity then skip this
    if (parseFloat(trade1Qty) < trade1.quantity) {
        if (trade1Qty < trade1.quantity * 0.5) {
    ...
    try {
        ...
        const tradeRes1 = await self.asyncNewOrder({ ...trade1 }, candidate.id + ' trade1 ' + candidate.ts)

        // If partially filled 50% quantity, then we use this 50% execute trade2 & trade3
        if (tradeRes1.status === 'PARTIALLY_FILLED' && tradeRes1.executedQty.sub(tradeRes1.origQty) > 0.5) {
            trade1.quantity = tradeRes1.executedQty
            this.computeQuantity({ ...trade1 }, trade2, trade3, trade1Qty, candidate)
            console.log(`<${candidate.id}> Partially filled more then 50% and recompute trade2, trade3's quantity`, trade2, trade3)

            tradeRes1.status = 'FILLED'
        }

        if (tradeRes1.status !== 'FILLED') {
            throw new Error('EXEC FAILED: order not filled')
        }

        console.log(`<${candidate.id}> [trade2]:`, 'symbol:', trade2);
        const tradeRes2 = await self.asyncNewOrder({ ...trade2 }, candidate.id + ' trade2 ' + candidate.ts, true);

        console.log(`<${candidate.id}> [trade3]:`, 'symbol:', trade3);
        const tradeRes3 = await self.asyncNewOrder({ ...trade3 }, candidate.id + ' trade3 ' + candidate.ts, true);
...
TradingCore.prototype.posCycleArbitrage = function (candidate) {
    ...
    this.computeQuantity(trade1, trade2, trade3, baseQuote.A)
    ...
    return { trade1, trade2, trade3, trade1Qty: baseQuote.A };
}
...
TradingCore.prototype.negCycleArbitrage = function (candidate) {
    ...
    return { trade1, trade2, trade3, trade1Qty: quoteMid.B };
}

TradingCore.prototype.computeQuantity = function (trade1, trade2, trade3, trade1Vol, candidate) {
    const trade2Origin = QuantityTools.trade2(trade1, trade2)
    const trade3Origin = QuantityTools.trade3(trade2, trade3)

    trade1.quantity = this.computeMiniQuantity(trade1.symbol, QuantityTools.trade1(trade1, trade1Vol));
    trade2.quantity = this.computeMiniQuantity(trade2.symbol, trade2Origin)
    trade3.quantity = this.computeMiniQuantity(trade3.symbol, trade3Origin)

    candidate.trading.trade2Margin = trade2Origin - trade2.quantity
    candidate.trading.trade3Margin = trade3Origin - trade3.quantity
...
TradingCore.prototype.asyncQueryOrder = async function (trade, orderId, cid, isOnly=false) {
    ...
    try {
        ...
        console.log(`<${cid}> Get error: ${e} with: ${trade} orderId: ${orderId}`)
        if (!isOnly) {
...
TradingCore.prototype.asyncNewOrder = async function (trade, cid, mustBeFilled = false) {
    ...
    try {
        ...
        if (e.code === -2010 && mustBeFilled && trade.quantity > 0.001) {
            console.log(`<${cid}> To 0.99%`)
            trade.quantity = trade.quantity.mul(0.99)
            return await this.asyncNewOrder(trade, cid, mustBeFilled)
        }
...
TradingCore.prototype.convertOrderToMarketPrice = async function (trade, cid) {
    ...
    return await this.asyncNewOrder(newTrade, cid, false)
}

TradingCore.prototype.cancelOrder = async function (cid, trade, orderId) {
    await this._exchange.cancelOrder({
        symbol: trade.symbol,
        orderId: orderId
    })

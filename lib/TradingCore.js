...
TradingCore.prototype.trading = async function (candidate) {
    ...
    try {
        if (trade1.side === 'BUY') {
            return await this.executeAsyncTrading(candidate, trade1, trade2, trade3)
        }

        return await this.executeTrading(candidate, trade1, trade2, trade3)
    } catch (e) {
        console.log(`%c <${candidate.id}> Arbitrage Failed:`, 'color: green', e);
    }
}

TradingCore.prototype.executeAsyncTrading = async function (candidate, trade1, trade2, trade3) {

    console.log(`<${candidate.id}> [trade1]:`, 'symbol:', trade1);

    // sync with trade1, say trade1 trade2 execute at same time
    const tradeRes1 = this.asyncNewOrder({ ...trade1 }, candidate.id + ' trade1 ' + candidate.ts, true)
...
TradingCore.prototype.executeTrading = async function (candidate, trade1, trade2, trade3) {

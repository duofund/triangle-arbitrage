...
function TradingCore(opts, currencyCore, exchange) {
  if (!(this instanceof TradingCore)) return new TradingCore(opts, currencyCore, exchange);
  ...
  for (let i=0;i<keys.length;i++){
    ...
    if (cand.hits >= this._minHitsThreshold){
      ...
      if (liveRate && liveRate.rate >= this._minQueuePercentageThreshold){
        ...
        // console.log('1. Trading candi:', cand);
        // console.log('2. liveRate:', liveRate);
        // console.log('3. Exchange:', self._exchange);
      }
    }
  }
};

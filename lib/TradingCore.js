...
TradingCore.prototype.trading = async function (candidate) {
    ...
    console.log(`<${candidate.id}> ===== ${candidate.dir} ====== tradeObj: `, tradeObj, candidate)
...
TradingCore.prototype.posCycleArbitrage = function (candidate) {
    ...
    trade1.quantity = this.computeQuantity(trade1);
    trade2.quantity = trade1.quantity.mul(1.0.sub(this._percentageOfFee)) // * trade2.b;
    trade3.quantity = trade2.quantity.mul(trade2.price).div(trade3.price).mul(1.0.sub(this._percentageOfFee));
...
TradingCore.prototype.computeQuantity = function (trade) {
    ...
    let quantity = this._mainCoinQuantityLimit.div(trade.price)

    if (this._ctrl.miniQuantity[trade.symbol] === 1) {
        return parseInt(quantity)
    }

    return quantity.betterToFixed(3)
}
...
TradingCore.prototype.negCycleArbitrage = function (candidate) {
    ...
    trade1.quantity = this.computeQuantity(trade1);
    trade2.quantity = (trade1.quantity.mul(trade1.price)).div(trade2.price).betterToFixed(2);

    if (trade2.symbol.indexOf('BNB') === 0) {
        trade2.quantity = parseInt(trade2.quantity)
    }

    trade3.quantity = trade2.quantity.mul(1.0.sub(this._percentageOfFee)).betterToFixed(2);

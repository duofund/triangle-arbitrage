...
TradingCore.prototype.trading = async function (candidate) {
    ...
    const { trade1, trade2, trade3, trade1Qty } = tradeObj
    ...
    try {
        ...
        if (tradeRes1.status === 'PARTIALLY_FILLED' && tradeRes1.executedQty.sub(tradeRes1.origQty) > 0.3) {
        ...
        this.computeAndSaveProfit(candidate.id, tradeObj, {
...
TradingCore.prototype.computeAndSaveProfit = function (cid, tradeObj, tradeRes) {
    utils.computeReturn(cid, tradeObj, tradeRes).then(res => {
        console.log(`<${cid}> [computeReturn] res:`, res)
...
TradingCore.prototype.negCycleArbitrage = function (candidate) {
    ...
    this.computeQuantity(trade1, trade2, trade3, quoteMid.B)
...
TradingCore.prototype.computeQuantity = function (trade1, trade2, trade3, trade1Vol) {
...
TradingCore.prototype.asyncNewOrder = async function (trade, cid, mustBeFilled = false) {
    ...
    try {
        ...
        if (e.code === -2010 && mustBeFilled && trade.quantity > 0.001) {
            console.log(`<${cid}> To 0.98%`)
            trade.quantity = trade.quantity * 0.98

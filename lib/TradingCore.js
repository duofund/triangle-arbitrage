...
function TradingCore(opts, ctrl) {
  ...
  this._arbitrage = opts.arbitrage;
...
TradingCore.prototype.processQueue = async function(queue, stream){
  ...
  for (let i=0;i<keys.length;i++){
    ...
    const tag = this.checkPosNeg(cand)
    return
...
TradingCore.prototype.checkPosNeg = function (candidate) {
    ...
    const markets = [ a, b, c ]
    const marketsMap = { [a.symbol]: a, [b.symbol]: b, [c.symbol]: c }
    const symbols = `${a.symbol}${b.symbol}${c.symbol}`

    const quote = this._arbitrage.start
    const mid = this._arbitrage.paths.filter(path => symbols.indexOf(quote + path) >= 0)
    const base = markets.filter(market => market.stepFrom !== quote && market.stepFrom !== mid)

...
CurrencyCore.getDynamicCandidatesFromStream = (stream, options, arrowTime) => {
  ...
  options.paths.map(path => {
    ...
    new Promise(resolve => {
      // CurrencyCore.getCandidatesFromStreamViaPath(stream, options.start, path, arrowTime);
  ...
  //   CurrencyCore.getCandidatesFromStreamViaPath(stream, options.start, options.paths[i], arrowTime);
...
CurrencyCore.startAllTickerStream = function(exchange){
  ...
  console.log('[startAllTickerStream] onAllTickers exchange.WS:', exchange.WS.onAllTickers)

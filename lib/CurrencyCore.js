...
CurrencyCore.getCandidatesFromStreamViaPath = function (stream, aPair, bPair) {
  var keys = {
    a: aPair.toUpperCase(),
    b: bPair.toUpperCase(),
    ...
    if (akeys[bPairTicker.key]){       
      ...
      if (stepC){
        ...
        if (comparison){
          ...
          triangle.ts = new Date().getTime()
          triangle.trading.dir && console.log(`%c [getCandidatesFromStreamViaPath] An Order been added <${triangle.ts}>`, 'color: #999999')

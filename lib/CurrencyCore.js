...
CurrencyCore.checkAndTrade = function (candidate, arrowTime, keys) {
  ...
  if (!this.checkIfTrade(keys, candidate)) return // console.log('Keys not trade:', keys)
...
CurrencyCore.checkIfTrade = function ({ a, b, c }, candidate) {
    if (candidate.ts - candidate.a.E > 260 || candidate.ts - candidate.b.E > 260 || candidate.ts - candidate.c.E > 260) {
        // console.log(`<${candidate.id}> Price is too order more then 260ms ts: ${candidate.ts} ws_ts: ${candidate.ws_ts}`)
        return false
    }

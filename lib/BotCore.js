...
module.exports = (ctrl) => {
  ...
  ctrl.storage.streamTick = (stream, streamID, arrowTime) => {
    ...
    ctrl.storage.candidates = ctrl.currencyCore.getDynamicCandidatesFromStream(stream, ctrl.options.arbitrage, arrowTime);
    ...
    // if (pairToTrade != 'none') {
    //   const { a, b, c } = pairToTrade
    //   console.log('%c GO TRADE :', 'color: red;', `a: ${a.s} ${a.tradeInfo.side} b: ${b.s} ${b.tradeInfo.side} c: ${c.s} ${c.tradeInfo.side}`);
    // }
  ...
  this.tradingCore = TradingCore(ctrl);
  ctrl.currencyCore = require('./CurrencyCore')(ctrl, this.tradingCore);

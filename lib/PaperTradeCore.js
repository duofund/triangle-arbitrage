...
PaperTradeCore.prototype.arbitrage = function (trade1, trade2, trade3, candidate) {
    ...
    resTrade1.status !== 'FILLED' && console.log('%c Error had happened', 'background-color: black; color: white;', resTrade2)

    const resTrade3 = this.openOrders(trade3);
    resTrade1.status !== 'FILLED' && console.log('%c Error had happened', 'background-color: black; color: white;', resTrade3)
    ...
    console.log(`%c <${candidate.id}> Profit: ${profit} ETH | ${(profit.div(resTrade1.quantity.mul(resTrade1.price)) * 100).toFixed(2)} %`, `background-color: ${profit > 0 ? 'red' : 'green'}; color: white;`, ` | Expect: ${candidate.rate}`)
}
...
PaperTradeCore.prototype.openOrders = function (trade) {
    ...
    const posOfKey = trade.s.indexOf(trade.stepFrom)

    let baseCur = trade.stepFrom // posOfKey === 0 ? trade.key : otherCur
    let quoteCur = trade.stepFrom // posOfKey === 0 ? otherCur :  trade.key

    if (posOfKey === 0) {
        quoteCur = trade.s.substring(trade.stepFrom.length, trade.s.length)
    } else {
        baseCur = trade.s.substring(0, posOfKey)
    }

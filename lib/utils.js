...
function computeReturn (candidate, tradeObject, tradeResults) {
    return new Promise((resolve) => {
        ...
        const trade1Qty = parseFloat(tradeRes1.executedQty) || parseFloat(trade1.quantity)
        const trade2Qty = parseFloat(tradeRes2.executedQty) || parseFloat(trade2.quantity)
        const trade3Qty = parseFloat(tradeRes3.executedQty) || parseFloat(trade3.quantity)
        ...
        if (trade1.side === 'BUY') {
            ...
            mergeMainQty = trade1Qty.div(trade1Price).sub(trade2Qty.mul(trade2Price))
        }

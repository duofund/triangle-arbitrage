...
function computeReturn (cid, tradeObject, tradeResults) {
    return new Promise((resolve) => {
        const fee = 0.003
        const { tradeRes1, tradeRes2, tradeRes3 } = tradeObject
        const startCoin = process.env.binanceStartingPoint
        const costQty = tradeRes1.symbol.indexOf(startCoin) === 0 ? tradeRes1.quantity : tradeRes1.quantity.mul(tradeRes1.price)
        const resultQty = tradeRes3.symbol.indexOf(startCoin) === 0 ? tradeRes3.quantity : tradeRes3.quantity.mul(tradeRes3.price)

...
function computeReturn (cid, tradeObject, tradeResults) {
    return new Promise((resolve) => {
        ...
        const costQty = trade1.symbol.indexOf(startCoin) === 0 ? trade1.quantity : trade1.quantity.div(trade1.price)
        const resultQty = trade3.quantity.mul(trade3.price)
        const profit = resultQty.sub(costQty)
        const profitPercentage = resultQty.sub(costQty).div(costQty).mul(100).betterToFixed(3)

        console.log(`${cid} ===== tradeResults:`, tradeResults)

        const msg = `[TA <${cid}> Success] Cost: ${costQty} ${startCoin} | Get: ${resultQty} ${startCoin} | Profit: ${profit} ${startCoin} (${profitPercentage} %)`
        console.log('Push to dingding: ', msg)
        dingDingPush(msg)
...
module.exports = {
    floatSub, floatMul, isBuyingCoin, findArbitrageRelationship, dingDingPush, computeReturn
};